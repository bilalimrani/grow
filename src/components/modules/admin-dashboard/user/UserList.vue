          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="user in users.data" v-bind:key="user.id">
                ...
                <td>{{user.lname}}</td>
                <td>{{user.email}}</td>
                <td :class="{'active' : user.insta_accounts[0].status=='ACTIVE', 'inactive' : user.insta_accounts[0].status=='INACTIVE'}"  v-if="user.insta_accounts.length">
              ...
              <td>
                ...
                <td>
                  <button v-if="user.insta_accounts.length" :disabled="user.updateInProgress" class="btn btn-secondary btn-block modal-button stat-act" @click="handleStatus(user.$index, user)">
                    <viralproof-loader :showLoader="user.updateInProgress" :message="`Do ${user.insta_accounts[0].status === 'INACTIVE' ? 'ACTIVE' : 'INACTIVE'}`" :loadingMessage="'Loading'"></viralproof-loader>
                </button>
                </td>
              </tr>
            </tbody>
